...
if(isset($_POST["update"])){
	...
	if ($stok < $jumlah) {
		...
		<script language="JavaScript">
			alert('Oops! melebihi jumlah stok ...');
			document.location='./';
		</script>
		<?php
	} else {
		$uprod = mysqli_query($conn, "UPDATE detailorder SET qty='$jumlah' WHERE idproduk='$kode' AND orderid='$orderidd'");
		if($uprod){
                //update stok
			$upstok= mysqli_query($conn, "UPDATE produk SET stok='$sisa' WHERE idproduk='$kode'");
			?>
			<script language="JavaScript">
				alert('Horee! update berhasil ...');
				document.location='./';
			</script>
			<?php
		}
